...
url = 'https://www.coupang.com/np/categories/184555?listSize=120&brand=&offerCondition=&filterType=&isPriceRange=false&minPrice=&maxPrice=&page=1&channel=user&fromComponent=N&selectedPlpKeepFilter=&sorter=bestAsc&filter=&rating=0'
# url
# url
# url
# url
...
try:
    ...
    elems = soup.find_all('a', class_='baby-product-link')
    print(len(elems))
    ...
    for i in range(len(elems)):
        # 0링크
        # print(elems[i]['href'])
        link = ('https://m.coupang.com' +
                elems[i]['href']).replace('/vp/', '/vm/').replace('&isAddedCart=', '')
        if url_code == True:
            link = short_url(link)
        ...
        if link.count('없음') == 0:
            ...
            try:
                discount_percentage = elems[i].find(
                    'span', class_='discount-percentage').text.strip()
            except:
