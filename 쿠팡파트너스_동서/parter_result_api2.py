...
for i in range(len(lists)):
    if int(lists[i]['quantity']) != 0:
        title_list.append([lists[i]['date'], str(lists[i]['productName']), str(int(lists[i]['quantity'])), str(int(lists[i]['gmv'])), str(int(lists[i]['commissionRate'])), str(int(lists[i]['commission']))])
#print(title_list)
...
try:
    ...
    uploadFiles = ['fn_list.csv', 'title_list.csv']
    for files in uploadFiles:
        with open(file=files, mode='rb') as wf:
            if files.count('.html') > 0:
                session.storbinary(files, wf)
            else:
