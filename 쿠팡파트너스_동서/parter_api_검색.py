...
# url
#정월대보름 서치
url = 'https://www.coupang.com/np/search?rocketAll=false&q=%EC%A0%95%EC%9B%94%EB%8C%80%EB%B3%B4%EB%A6%84&brand=&offerCondition=&filter=&availableDeliveryFilter=&filterType=&isPriceRange=false&priceRange=&minPrice=&maxPrice=&page=1&trcid=&traid=&filterSetByUser=true&channel=user&backgroundColor=&searchProductCount=2889&component=&rating=0&sorter=scoreDesc&listSize=72'
#이미지 수집 문제 있음
...
elems = soup.find_all('li', class_='search-product')
#print(len(elems))
...
try:
    for i in range(len(elems)):
        ...
        # print(elems[i]['href'])
        link = 'https://m.coupang.com' + elems[i].find('a')['href'].replace('/vp/', '/vm/').replace('&isAddedCart=', '')
        print(link)
        if url_code == True:
            link = short_url(link)
        ...
        if link.count('없음') == 0:
            ...
            # 1상품 이미지
            thumbnail = elems[i].find('dt', class_='image')
            thumbnail = 'https:'+thumbnail.find('img')['src']
            print(thumbnail)
            if thumbnail.count('http') == 0:
                time.sleep(1000)

            # 2상품명
            descriptions = elems[i].find('dd', class_='descriptions')
            title = descriptions.find('div', class_='name').text.replace(
            ...
            # 3할인율 (옵)
            try:
                discount_percentage = elems[i].find(
                    'span', class_='instant-discount-rate').text.strip()
            except:
                discount_percentage = 'none'

            # 4할인전 가격 (옵)
            try:
                base_price = elems[i].find(
                    'del', class_='base-price').text.strip()
            except:
                base_price = ''

            # 5판매가
            descriptions = elems[i].find('dd', class_='descriptions')
            price = descriptions.find('strong').text
            ...
            # 7별수 (옵)
            try:
                rating_star = elems[i].find(
                    'div', class_='rating-star').text[:4].strip()
            except:
                rating_star = 'none'

            # 8리뷰수 (옵)
            try:
                rating_total_count = elems[i].find(
                    'span', class_='rating-total-count').text.replace('(', '').replace(')', '').strip()
            except:
